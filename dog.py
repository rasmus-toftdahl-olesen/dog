...
def list_from_config_entry(entry: str) -> list:
    var_list = entry.split(',')
    return [i.strip() for i in var_list]


def get_user_env_vars(config_user_env_vars: str, allow_empty: bool) -> dict:
...
def generate_env_arg_list(config: DogConfig) -> list:
    args = []
...
def docker_compose_run(config: DogConfig) -> int:
    ...
    args = ['sudo'] if config[SUDO_OUTSIDE_DOCKER] else []
    args.extend(['docker-compose', '-f', str(Path(config[DOG_CONFIG_PATH]) / config[DOCKER_COMPOSE_FILE])])
    if config[VERBOSE]:
        args += ['--verbose',
                 '--log-level', 'DEBUG'
                 ]
    ...
    try:
        ...
        proc = subprocess.run(args, env={'DOG_HOSTNAME': config[HOSTNAME]}, stderr=stderr_out)
        return proc.returncode
    except KeyboardInterrupt:
        print('Dog received Ctrl+C')
        return -1
...
def update_dependencies_in_config(config: DogConfig):
    ...
    volumes = {}
    for inside, outside in config[VOLUMES].items():
        new_inside = config[HOME] + inside[5:] if inside.startswith('$' + HOME) else inside
        new_outside = home_path + outside[1:] if outside.startswith('~') else outside
