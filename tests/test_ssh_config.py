...
@pytest.mark.skipif(
    ...
    reason='This test does not work on GitHub actions since it uses SSH authentication',
)
@pytest.mark.skipif(
    'TEAMCITY_PROJECT_NAME' in os.environ,
    reason=(
        'This test only works locally',
        ' - and probably only for rasmus-toftdahl-olesen!',
    ),
)
